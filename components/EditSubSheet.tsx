...
export default function EditSubSheet({
  id,
  onClose,
}: {
  id: string | null;
  ...
  useEffect(() => {
    ...
    if (activeSub) {
      setF({
        ...
        startDate:
          activeSub.startDate || new Date().toISOString().split("T")[0],
        isTrial: activeSub.isTrial || false,
        trialDays: String(
          activeSub.trialEndDay ? trialDaysLeft(activeSub.trialEndDay) : 14,
        ),
        customNum: String(activeSub.customNum || 2),
        customUnit: activeSub.customUnit || "months",
        active: activeSub.active ?? true,
        reminderDays:
          activeSub.reminderDays != null ? String(activeSub.reminderDays) : "",
      });
  ...
  const save = () => {
    ...
    updateSub({
      ...
      trialDecision:
        f.isTrial && original.trialDecision === "none"
          ? "pending"
          : f.isTrial
            ? original.trialDecision
            : "none",
      ...
      >
        ...
        <View
          style={[
            s.trialBox,
            {
              backgroundColor: f.active ? C.bgSub : C.redBg,
              borderColor: f.active ? "rgba(0,0,0,0.04)" : C.redLine,
              marginBottom: 20,
            },
          ]}
        >
          <View style={{ flex: 1 }}>
            <Text style={[s.trialLabel, { color: f.active ? C.t1 : C.red }]}>
              {t("editSub.active")}
            </Text>
            <Text style={{ fontSize: 12, color: C.t3 }}>
              {t("editSub.toggleCancel")}
            </Text>
          </View>
          <Switch
            value={f.active}
        ...
        <View
          style={[
            s.trialBox,
            {
              ...
              borderColor: f.isTrial ? C.redLine : "transparent",
            },
          ]}
        >
          <View style={{ flex: 1 }}>
